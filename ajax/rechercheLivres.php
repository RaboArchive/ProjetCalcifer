...
if(isset($data)){
  ...
  $requeteLivreNotice = "SELECT distinct titre,auteur,image FROM livre L, LIVREDEPOSE LD WHERE L.ISBN=LD.ISBN AND (L.titre LIKE \"%$data%\" OR L.auteur LIKE \"%$data%\")";
  $requeteLivreDepose = "SELECT distinct titre,auteur, image FROM livre L, LIVREDEPOSE LD WHERE L.ISBN not in (select li.isbn from livre li ,livredepose lid where li.isbn=lid.isbn) AND (L.titre LIKE \"%$data%\" OR L.auteur LIKE \"%$data%\")";
