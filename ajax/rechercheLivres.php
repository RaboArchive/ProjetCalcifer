...
if(isset($data)){
  ...
  $requeteLivreDepose = "SELECT distinct titre,auteur,image,L.isbn FROM livre L, LIVREDEPOSE LD, user U WHERE L.ISBN not in (select li.isbn from livre li ,livredepose lid where li.isbn=lid.isbn) AND (L.titre LIKE \"%$data%\" OR L.auteur LIKE \"%$data%\")";
